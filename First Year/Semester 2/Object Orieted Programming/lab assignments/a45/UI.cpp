...
{
	dynamic_array = DynamicArray<TrenchCoat>(100); //aici imi apeleaza assingnment operator, pt ca il am deja definit in ui.h
	this->shopping_basket = DynamicArray<TrenchCoat>(100);
}
...
void User_Interface::generate_10_random_trench_coats()
{
	...
	dynamic_array.add_element(trench2);
	dynamic_array.add_element(trench3);
	dynamic_array.add_element(trench4);
	dynamic_array.add_element(trench5);
	dynamic_array.add_element(trench6);
	dynamic_array.add_element(trench7);
	dynamic_array.add_element(trench8);
	dynamic_array.add_element(trench9);
	dynamic_array.add_element(trench10);
	...
	while(1)
	{
		...
		if (person == 'a')
		{
			...
			while (1) {
				...
				if (admin_option == 1)
				{
					dynamic_array.display_all_elements_in_repo();
				}
				else if(admin_option == 2 || admin_option == 3 || admin_option == 4)
				{
					...
					std::string colour;
					std::cin >> colour;
					int position_of_trench_coat_in_repo = dynamic_array.search_element_in_repo(size, colour);
					if (admin_option == 2)
					{
						if (position_of_trench_coat_in_repo != -1)
						{
							int quantity_of_trench_coat = dynamic_array.get_element(position_of_trench_coat_in_repo).get_quantity();
							dynamic_array.get_element(position_of_trench_coat_in_repo).set_quantity(quantity_of_trench_coat++);
							...
							dynamic_array.add_element(new_trench_coat);
						}
					...
					else if (admin_option == 3)
					{
						...
						else
						{
							if (position_of_trench_coat_in_repo == -1)
								...
								dynamic_array.delete_element(position_of_trench_coat_in_repo);
						}
					}
							...
							else
							{
								...
								while (1)
								{
									...
									if (option_to_update == 'a')
									{
										...
										int new_price;
										std::cin >> new_price;
										dynamic_array.update_price(new_price,position_of_trench_coat_in_repo);
									...
									else if (option_to_update == 'b')
									{
										...
										dynamic_array.update_quantity(new_quantity, position_of_trench_coat_in_repo);
									}
									else if (option_to_update == 'c')
									{
										...
										dynamic_array.update_photograph(new_photo_link, position_of_trench_coat_in_repo);
									}
		...
		else if (person == 'b')
		{
			...
			while (1)
			{
				...
				if (user_option == 1)
				{
					...
					char option = 'i';
					while (option != 'e')
						if (strcmp(dynamic_array.get_element(i).get_size().c_str(), size.c_str()) == 0)
						{
							std::cout<<dynamic_array.get_element(i).display_as_string();
							std::cout << "\n Press 'i' to display the link of the image of the trench coat, or press '-' to skip this step";
							std::cout <<"\n>";
							char image;
							std::cin >> image;
							if (image == 'i')
								dynamic_array.get_element(i).ShowImage();
							else if (image == '-')
								std::cout << "\n";
							...
							if (add == '+')
							{
								shopping_basket.add_element(dynamic_array.get_element(i));
								total_price = total_price + dynamic_array.get_element(i).get_price();
							}
							if (add == '-' || add == '+')
							{
				...
				else if (user_option == 2)
				{
					...
					shopping_basket.display_all_elements_in_repo();
